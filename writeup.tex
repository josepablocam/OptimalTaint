...
& \mbox{if } \cprime={\sf while\ (\bexp)\ \{ \cdprime \}} \\
	\nocommand
  & \mbox{if } \cprime = {\sf \textnormal{$\overline{\sf z}$ := iexp}} \\
	\cprime
    & \mbox{otherwise} \\
  \end{cases}
\]
...
$$
That is, {\sf c} and {\sf c'} are semantically equivalent up to the values assigned to shadow values.
...
\newcommand{\flow}[3]{\textsf{#1 $\stackrel{\sf #2}{\rightarrow}$ #3}}
\newcommand{\ivar}{\textsf{i}}
\newcommand{\jvar}{\textsf{j}}
\newcommand{\kvar}{\textsf{k}}
\section{The Tainting Property}

Given program \prog\ and initial state $\sigma_0$, taint propagation is captured by graph $G = f(\emptyset)$, such that $t(\nocommand\ ;\ \prog,\sigma_0) \models f$, according to the following rules:
\[
\infer{t(\nocommand,\sigma) \models \lambda G.\ G \cup \{ \flow{i}{i}{i} \colon \ivar \in {\sf dom}(\sigma) \}}{\sigma = \sigma_0}
\]
\[
\infer{t(\nocommand,\sigma) \models \lambda G.\ G}{\sigma \neq \sigma_0}
\]
\[
\infer{t({\sf i := j},\sigma) \models \lambda G.\ G \cup \{ \flow{j}{k}{i} \colon \exists {\sf l}.\ \flow{l}{k}{j} \in G \}}{}
\]
\[
\infer{t({\sf i := j\ aop\ k},\sigma) \models \lambda G.\ G \cup \{ \flow{j}{n}{i} \colon \exists {\sf l}.\ \flow{l}{n}{j} \in G \} \cup \{ \flow{k}{n}{i} \colon \exists {\sf l}.\ \flow{l}{n}{k} \in G \}}{}
\]
\[
\infer{t({\sf i := \textnormal{$n$}},\sigma) \models \lambda G.\ G \setminus \{ \flow{j}{k}{i} \colon \jvar,\kvar \in {\sf dom}(\sigma) \}}{}
\]
\[
\infer{t(\prog,\sigma) \cdot t(\progp,\sigma') \models f' \cdot f}{t(\prog,\sigma) \models f,
	t(\progp,\sigma') \models f',\lsyn \prog \rsyn \sigma = \sigma'}
\]

We are interested to answer queries of the form $\ivar \leadsto \jvar$. As we state below, this is straightforward to evaluate given $G$.

\begin{lem}
	Given program \prog\ and initial state $\sigma_0$, let $t(\nocommand\ ;\ \prog, \sigma_0) \models f$ and $G=f(\emptyset)$. Then
	$$
		\ivar \leadsto \jvar \Longleftrightarrow \exists {\sf l}.\ \flow{l}{\ivar}{\jvar} \in G
	$$
	\begin{proof}
		Need proof ...
	\end{proof}
